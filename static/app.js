...
function login() {
  $("#app").innerHTML =
    `<main class="center"><form class="card" id="login"><div class="brand"><span class="logo">↗</span> ClawMio</div><div class="eyebrow">LOCAL AGENT WORKSPACE</div><h1>让微信连接执行力</h1><p class="muted">登录管理后台，管理微信 Bot 与百炼 Managed Agent 会话。</p><label for="password">管理密码</label><input id="password" type="password" autocomplete="current-password" required placeholder="输入本地管理密码"><p class="muted">使用部署时 .env 中配置的 ADMIN_PASSWORD 登录</p><p class="error-text" id="login-error"></p><button class="full">进入工作台 →</button></form></main>`;
...
function billingLabel(state) {
  return ({pending:"待结算",settled:"已结算",unpriced:"未配置单价，不计费",not_submitted:"未提交，不计费",rejected:"欠费拒绝",local_command:"本地指令，不计费"})[state] || "历史记录，不补扣";
}
...
async function bindingView(ticket, publicPage) {
  try {
    ...
    if (publicPage)
      target.innerHTML = `<main class="center"><div class="card"><div class="brand"><span class="logo">↗</span> ClawMio</div>${inside}</div></main>`;
